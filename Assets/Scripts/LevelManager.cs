...
public class LevelManager : MonoBehaviour {
	...
	public void Setting(){
		if(popActive){
			...
			SoundSlider.GetComponent<Slider> ().value = PlayerPrefs.GetFloat("musicVolume");
	...
	public void LoadNewLevel(string sceneName, float delay = loadSceneDelay) {
		...
		if(GetWorldName (SceneManager.GetActiveScene ().name) == "World 1-5"){
			LoadSceneDelay("Story 2", delay);
		}
		else{
